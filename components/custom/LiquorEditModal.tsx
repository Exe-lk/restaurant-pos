...
const CustomerEditModal: FC<ICustomerEditModalProps> = ({ id, isOpen, setIsOpen }) => {
	...
	const [liquor, setLiquor] = useState<any>();
        ...
        useEffect(() => {
            const fetchData = async () => {
                try {
                    ...
                    await setLiquor(firebaseData[0]);
	...
	const formik = useFormik({
		...
		onSubmit: async (values) => {
			try {
				...
				const imgurl: any = await handleUploadimage()||liquor?.imageurl;
